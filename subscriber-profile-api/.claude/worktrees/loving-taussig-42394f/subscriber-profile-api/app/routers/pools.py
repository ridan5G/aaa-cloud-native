...
def _compute_pool_bounds(subnet: str, start_ip: Optional[str], end_ip: Optional[str]):
    """Return (stored_start, stored_end) without materializing the full IP list."""
    try:
    ...
    if start_ip and end_ip:
        ...
        # Default: allocate from first host up to (but not including) last host (gateway).
...
async def list_pools(
    ...
    rows = await conn.fetch(
        ...
        SELECT p.pool_id::text, p.account_name, p.pool_name AS name,
               ...
               (
                   (p.end_ip - p.start_ip - p.next_ip_offset)
                   + COALESCE(avail.cnt, 0)
               )::bigint AS available,
               COALESCE(alloc.cnt, 0)::int AS allocated,
               (p.end_ip - p.start_ip)::bigint AS total
        FROM ip_pools p
        JOIN routing_domains rd ON rd.id = p.routing_domain_id
        LEFT JOIN (
