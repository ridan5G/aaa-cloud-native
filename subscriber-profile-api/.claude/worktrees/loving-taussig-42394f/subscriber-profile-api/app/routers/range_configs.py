...
async def _check_pool_capacity(
    ...
    async def _pool_available(pid: str) -> int:
        row = await conn.fetchrow(
            """
            SELECT (p.end_ip - p.start_ip - p.next_ip_offset)
                   + (SELECT COUNT(*) FROM ip_pool_available WHERE pool_id = $1::uuid)
                   AS available
            FROM ip_pools p WHERE p.pool_id = $1::uuid
            """,
            pid,
        )
    ...
    if ip_resolution not in ("imsi_apn", "iccid_apn"):
        available = await _pool_available(default_pool)
        ...
        for pid, required in pools_needed.items():
            available = await _pool_available(pid)
