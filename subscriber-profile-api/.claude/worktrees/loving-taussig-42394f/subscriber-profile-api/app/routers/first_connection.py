...
async def _allocate_ip(conn, pool_id: str) -> Optional[str]:
    """Claim one IP from the pool. Returns None if pool is exhausted.

    Phase 1: pop a recycled IP from ip_pool_available (released subscribers).
    Phase 2: atomically claim the next unallocated IP via next_ip_offset.
    This avoids pre-populating millions of rows for large subnets.
    """
    recycled = await conn.fetchval(
        """
    ...
    # Claim next offset; capacity = end_ip - start_ip (last host is reserved gateway).
    return await conn.fetchval(
        """
        UPDATE ip_pools
        SET next_ip_offset = next_ip_offset + 1
        WHERE pool_id = $1::uuid
          AND next_ip_offset < (end_ip - start_ip)
        RETURNING host(start_ip + (next_ip_offset - 1))
        """,
        pool_id,
    )
