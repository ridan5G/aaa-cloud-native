...
//   LOOKUP_URL               aaa-lookup-service primary base URL      (default: http://aaa-lookup-service:8081/v1)
//   LOOKUP_URL_SECONDARY     aaa-lookup-service secondary base URL    (optional; enables region failover)
...
struct Config {
    ...
    std::string lookupUrlSecondary;  // optional — empty means no failover
    int         workerThreads        = 8;
    ...
    void load() {
        ...
        if (auto* v = getenv("LOOKUP_URL_SECONDARY"))    lookupUrlSecondary   = v;
