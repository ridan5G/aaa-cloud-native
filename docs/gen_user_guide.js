  ...
  sections: [
    ...
    // Body
    {
      ...
      children: [
        ...
        hdr("1. Kubernetes Deployment"),
        para("This section covers end-to-end deployment of the AAA Cloud-Native Platform on Kubernetes using the project Makefile. All operational commands are driven by make targets; no manual kubectl or helm invocations are required for the standard workflow."),
        spacer(60),

        hdr("1.1 Prerequisites", HeadingLevel.HEADING_2),
        para("Choose the environment that matches your setup:"),
        table(
          ["Requirement", "Docker Desktop path", "k3d path"],
          [
            ["Container runtime", "Docker Desktop (with Kubernetes enabled)", "Docker Desktop or Docker Engine"],
            ["Local cluster", "Docker Desktop built-in Kubernetes", "k3d v5+ (make cluster-up installs it)"],
            ["CLI tools", "kubectl, helm, make, git", "kubectl, helm, make, git, k3d"],
            ["Image registry", "Not required (pullPolicy: Never)", "k3d local registry (created by make cluster-up)"],
            ["Node.js (optional)", "For regenerating docs only", "For regenerating docs only"],
          ],
          [2800, 3000, 3560]
        ),
        spacer(60),
        noteBox("First-time setup", "Before bootstrapping, copy the example env files and fill in required secrets:\n  cp subscriber-profile-api/.env.example subscriber-profile-api/.env\n  cp aaa-regression-tester/.env.example aaa-regression-tester/.env\nAt minimum set RADIUS_SECRET — it has no default and the stack will refuse to start without it."),
        spacer(80),

        hdr("1.2 First-Time Bootstrap", HeadingLevel.HEADING_2),
        para("Run one of the following single commands to stand up the entire stack from scratch. Each command builds all images, configures the cluster, installs the Helm charts, and initialises the database."),
        spacer(60),
        hdr("Docker Desktop (recommended for laptops)", HeadingLevel.HEADING_3),
        para("Requires Docker Desktop with Kubernetes enabled in Settings > Kubernetes."),
        code("make setup"),
        para("Runs in order: cnpg-install  →  nginx-install  →  hosts  →  build-cpp-bases  →  build-all  →  dep-update  →  prom-crds  →  deploy  →  db-init"),
        spacer(60),
        hdr("k3d (lightweight multi-node clusters)", HeadingLevel.HEADING_3),
        para("Creates a local k3d cluster, builds and pushes images to the local registry, then deploys."),
        code("make bootstrap"),
        para("Runs in order: cluster-up  →  hosts  →  build-cpp-bases  →  build-all  →  push-all  →  dep-update  →  prom-crds  →  deploy  →  db-init"),
        spacer(60),
        noteBox("build-cpp-bases is slow the first time", "The make build-cpp-bases step compiles vcpkg dependencies and may take 10–20 minutes on first run. Subsequent builds skip it unless vcpkg.json changes. Run make build-cpp-bases separately if you update C++ dependencies."),
        spacer(80),

        hdr("1.3 Image Build Targets", HeadingLevel.HEADING_2),
        para("After the initial bootstrap, use these targets for incremental image builds during development:"),
        table(
          ["Command", "Description"],
          [
            ["make build-cpp-bases", "Build vcpkg/CMake base images. Run once, or after changing vcpkg.json"],
            ["make build-all", "Build all 5 service images (requires build-cpp-bases to have run first)"],
            ["make push-all", "Push all images to the registry (required for k3d; not needed for Docker Desktop)"],
            ["make build-api", "Rebuild subscriber-profile-api image and trigger a rolling restart in the cluster"],
            ["make build-lookup", "Rebuild aaa-lookup-service image and trigger a rolling restart in the cluster"],
          ],
          [3200, 6160]
        ),
        spacer(60),
        noteBox("Tip", "For rapid Python code iteration, make build-api rebuilds only the API image and immediately restarts the pod — no full make build-all needed. For C++ changes, make build-lookup does the same for aaa-lookup-service."),
        spacer(80),

        hdr("1.4 Deploy and Upgrade", HeadingLevel.HEADING_2),
        para("After images are built, deploy or upgrade the platform with:"),
        code("# Development mode (default)"),
        code("make deploy"),
        code(""),
        code("# High-availability mode (anti-affinity, PDBs, 3 PgBouncer replicas)"),
        code("make deploy-ha"),
        spacer(60),
        para("Both targets run helm upgrade --install with a 10-minute timeout and automatically run db-init afterwards to apply any pending schema changes. Re-running deploy is safe and idempotent."),
        spacer(60),
        table(
          ["Values file", "Use case", "Key differences"],
          [
            ["values-dev.yaml", "Local development and CI", "Single replicas, pullPolicy: Never, dev-grafana password, JWT_SKIP_VERIFY"],
            ["values-ha.yaml", "Staging and production simulation", "Anti-affinity rules, PodDisruptionBudgets, 3 PgBouncer replicas, stricter resource limits"],
          ],
          [2200, 3000, 4160]
        ),
        spacer(80),

        hdr("1.5 Database Initialisation", HeadingLevel.HEADING_2),
        para("Schema creation and grants are handled by a dedicated script that is idempotent — safe to run multiple times:"),
        code("make db-init"),
        spacer(60),
        para("This runs bash scripts/db-init.sh which creates all tables (if they do not exist), creates the aaa_app role, and grants the necessary privileges. It is called automatically by make deploy and make bootstrap, but can be run independently after a cluster restart or schema change."),
        spacer(80),

        hdr("1.6 Verify the Deployment", HeadingLevel.HEADING_2),
        code("make status"),
        para("Displays pods, services, ingress objects, PVCs, the CNPG cluster state, and any active Jobs in the aaa-platform namespace. All pods should show Running or Completed."),
        spacer(60),
        para("After a successful deploy, services are reachable via Ingress at these local URLs (added to /etc/hosts by make hosts):"),
        table(
          ["URL", "Service", "Notes"],
          [
            ["http://ui.aaa.localhost", "aaa-management-ui", "Operator dashboard"],
            ["http://provisioning.aaa.localhost/v1", "subscriber-profile-api", "Provisioning REST API"],
            ["http://lookup.aaa.localhost/health", "aaa-lookup-service", "Health endpoint"],
            ["http://grafana.aaa.localhost", "Grafana", "admin / dev-grafana (dev mode)"],
            ["http://prometheus.aaa.localhost", "Prometheus", "Metrics scrape target browser"],
          ],
          [3200, 2400, 3760]
        ),
        spacer(80),

        hdr("1.7 Port Forwards (Alternative to Ingress)", HeadingLevel.HEADING_2),
        para("If you prefer direct port-forward access (e.g., for debugging or when Ingress is not configured):"),
        table(
          ["Command", "Local Port", "Service"],
          [
            ["make port-forward-api", "8080", "subscriber-profile-api"],
            ["make port-forward-lookup", "8081", "aaa-lookup-service"],
            ["make port-forward-db", "5432", "PostgreSQL (primary)"],
            ["make port-forward-ui", "8090", "aaa-management-ui"],
            ["make port-forward-grafana", "3000", "Grafana"],
            ["make port-forward-prometheus", "9090", "Prometheus"],
            ["make port-forward-pgbouncer", "5432", "PgBouncer connection pooler"],
          ],
          [3200, 1600, 4560]
        ),
        spacer(60),
        noteBox("Tip", "Run each port-forward in a separate terminal, or in the background with & (Linux/macOS). They remain active until you Ctrl+C or close the terminal."),
        spacer(80),

        hdr("1.8 Run Regression Tests", HeadingLevel.HEADING_2),
        para("The regression suite runs as a Kubernetes Job and validates the full platform end-to-end, including RADIUS authentication flows:"),
        code("make test"),
        spacer(60),
        para("To capture RADIUS packet traces during the test run:"),
        code("make test PCAP=true"),
        spacer(60),
        para("Monitor progress with:"),
        code("make status        # shows job pod status"),
        code("make logs          # tails the test runner pod"),
        spacer(60),
        noteBox("RADIUS_SECRET must match", "The regression tests send live RADIUS packets. The RADIUS_SECRET in aaa-regression-tester/.env must match the value used during deploy (defaults to testing123 in dev mode). Set it explicitly in both places for non-default deployments."),
        spacer(80),

        hdr("1.9 Day-2 Operations", HeadingLevel.HEADING_2),
        table(
          ["Command", "Description"],
          [
            ["make logs", "Tail logs from all platform pods (combined stream)"],
            ["make logs-api", "Tail subscriber-profile-api logs only"],
            ["make logs-lookup", "Tail aaa-lookup-service logs only"],
            ["make logs-ui", "Tail aaa-management-ui logs only"],
            ["make db-flush-stale", "Truncate sim_profiles, imsi2sim, imsi_apn_ips, sim_apn_ips (dev only — non-reversible)"],
            ["make helm-unlock", "Clear a stuck Helm release lock (failed upgrade left pending state)"],
            ["make uninstall", "Remove all Helm releases, the CNPG cluster, and PVCs"],
            ["make clean", "Full teardown: uninstall everything + destroy the k3d cluster"],
          ],
          [3200, 6160]
        ),
        spacer(60),
        noteBox("Warning", "make db-flush-stale truncates subscriber and IP assignment tables without confirmation. Use only in development environments. make uninstall deletes PVCs — all database data will be lost.", ORANGE_BG),
        spacer(80),

        // ── 2. Getting Started ─────────────────────────────────────────
        hdr("2. Getting Started"),
        para("This guide covers day-to-day operations for provisioning subscribers, managing IP address pools, and monitoring the AAA Cloud-Native Platform. It is intended for telecom network engineers and operations teams. For first-time deployment instructions see Section 1."),
        spacer(60),
        ...
        hdr("2.1 Prerequisites", HeadingLevel.HEADING_2),
        ...
        hdr("2.2 API Access", HeadingLevel.HEADING_2),
        ...
        hdr("2.3 Health Check", HeadingLevel.HEADING_2),
        ...
        hdr("3.1 Create an IP Pool", HeadingLevel.HEADING_2),
        ...
        hdr("3.2 Check Pool Statistics", HeadingLevel.HEADING_2),
        ...
        hdr("3.3 Suspend and Reactivate a Pool", HeadingLevel.HEADING_2),
        ...
        hdr("4.1 Single-IMSI Range Config", HeadingLevel.HEADING_2),
        ...
        hdr("4.2 Multi-IMSI SIM Range Config", HeadingLevel.HEADING_2),
        ...
        hdr("4.3 Per-APN Pool Overrides", HeadingLevel.HEADING_2),
        ...
        hdr("4.4 Auto-Allocation Scenarios", HeadingLevel.HEADING_2),
        ...
        hdr("5.1 Create a Profile (Profile Mode B - IMSI-level IP)", HeadingLevel.HEADING_2),
        ...
        hdr("5.2 Create a Profile (Profile Mode C - APN-specific IP)", HeadingLevel.HEADING_2),
        ...
        hdr("5.3 Create a Profile (Profile Mode A - Card-level IP)", HeadingLevel.HEADING_2),
        ...
        hdr("5.4 Look Up an Existing Profile", HeadingLevel.HEADING_2),
        code("# By device UUID"),
        ...
        hdr("5.5 Update a Profile", HeadingLevel.HEADING_2),
        ...
        new Paragraph({ children: [new PageBreak()] }),
        hdr("6. Subscriber Lifecycle Management"),

        hdr("6.1 Suspend a Subscriber", HeadingLevel.HEADING_2),
        ...
        hdr("6.2 Reactivate a Subscriber", HeadingLevel.HEADING_2),
        code("PATCH /v1/profiles/550e8400-..."),
        code("{\"status\": \"active\"}"),
        spacer(80),

        hdr("6.3 Terminate a Subscriber", HeadingLevel.HEADING_2),
        ...
        hdr("6.4 Manage IMSIs on a Profile", HeadingLevel.HEADING_2),
        ...
        hdr("7.1 JSON Bulk Upload", HeadingLevel.HEADING_2),
        ...
        hdr("7.2 CSV Bulk Upload", HeadingLevel.HEADING_2),
        ...
        hdr("7.3 Monitor a Bulk Job", HeadingLevel.HEADING_2),
        ...
        new Paragraph({ children: [new PageBreak()] }),
        hdr("7.4 Profile Structure by Scenario", HeadingLevel.HEADING_2),
        ...
        new Paragraph({ children: [new PageBreak()] }),
        hdr("8. Monitoring First-Connection Allocation"),
        ...
        hdr("8.1 What Happens at First Connection", HeadingLevel.HEADING_2),
        ...
        hdr("8.2 Key Metrics to Watch", HeadingLevel.HEADING_2),
        table(
        ...
        hdr("8.3 Verify a Provisioned Subscriber After First Connection", HeadingLevel.HEADING_2),
        ...
        hdr("8.4 aaa-radius-server Integration Points", HeadingLevel.HEADING_2),
        ...
        // ── 8. Common Tasks ─────────────────────────────────────────────
        new Paragraph({ children: [new PageBreak()] }),
        hdr("9. Common Operational Tasks"),

        hdr("9.1 List All Pools for an Account", HeadingLevel.HEADING_2),
        code("GET /v1/pools?account_name=Melita"),

        hdr("9.2 Find a Profile by ICCID", HeadingLevel.HEADING_2),
        code("GET /v1/profiles?iccid=8944501012345678901"),
        spacer(80),

        hdr("9.3 Update a Range Config Pool", HeadingLevel.HEADING_2),
        ...
        hdr("9.4 Expand IP Capacity for a Subscriber Segment", HeadingLevel.HEADING_2),
        ...
        hdr("9.5 Perform a Migration Import", HeadingLevel.HEADING_2),
        ...
        hdr("10.1 Common Validation Rules", HeadingLevel.HEADING_2),
        table(
          ["Field", "Rule"],
          [
        ...
        new Paragraph({ children: [new PageBreak()] }),
        hdr("11. Local Development Setup"),

        hdr("11.1 Bootstrap the Full Stack", HeadingLevel.HEADING_2),
        ...
        hdr("11.2 Useful Make Targets", HeadingLevel.HEADING_2),
        table(
          ["Command", "Description"],
          [
        ...
        hdr("11.3 Connect to the Database Directly", HeadingLevel.HEADING_2),
        ...
        hdr("11.4 Development Auth (JWT Skip)", HeadingLevel.HEADING_2),
        ...
        hdr("11.5 Run Regression Tests Locally (Docker Compose)", HeadingLevel.HEADING_2),
