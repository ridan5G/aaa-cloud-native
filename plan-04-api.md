...
| Property | Value |
|---|---|
| Callers | Operators, BSS/OSS, migration scripts, UI, FreeRADIUS (first-connection fallback) |
...
| Method | Path | Description | Success | Notes |
|---|---|---|---|---|
| `POST` | `/iccid-range-configs` | Create ICCID range + validate `imsi_count` | 201 `{id}` | Does not create child IMSI ranges |
...
| `POST` | `/iccid-range-configs/{id}/imsi-slots` | Add a child IMSI range (one slot) | 201 `{range_config_id}` | Validates cardinality equality |
...
### First-Connection Allocation (FreeRADIUS Fallback)

Called by FreeRADIUS when `aaa-lookup-service` returns 404 for an unknown IMSI.
This is the only write path that runs in the AAA authentication flow.

| Method | Path | Description | Success | Error |
|---|---|---|---|---|
| `POST` | `/first-connection` | Allocate IP and permanently create subscriber profile | 200 `{"static_ip":"..."}` | 404 not in range; 503 pool exhausted |

**Note:** Returns 200 (not 201) so FreeRADIUS can use the same response handler as
the `aaa-lookup-service` 200 path. The caller cannot tell whether the IP was freshly
allocated or had previously existed.
...
### First-Connection Allocation (FreeRADIUS → provisioning)
...
2. Thread pool picks up job → processes profiles in batches of 1000
   For each batch:
     ...
     b. INSERT INTO subscriber_profiles ON CONFLICT (device_id) DO UPDATE
     c. INSERT INTO subscriber_imsis ON CONFLICT (imsi) DO NOTHING
     d. INSERT INTO subscriber_apn_ips ON CONFLICT DO NOTHING
...
```
device_id, iccid, account_name, status, ip_resolution, imsi_1, static_ip_1, pool_id_1,
imsi_2, static_ip_2, pool_id_2, ...
```

(Flat structure — multiple IMSI columns per row, up to a documented maximum of 10 IMSIs per row.
Profiles with more than 10 IMSIs must use the JSON bulk endpoint instead.)

---

## First-Connection Allocation — Internals

`POST /first-connection` is the write path that runs in the AAA flow when FreeRADIUS
...
comes **entirely from the range config**, not from the request body. FreeRADIUS passes
...
3. Branch on iccid_range_id:
   ...
   BEGIN
     ...
     INSERT subscriber_profiles (iccid=NULL, account_name, ip_resolution=$ip_resolution)
     INSERT subscriber_imsis (imsi, device_id, priority=1)

     IF ip_resolution IN ('imsi', 'imsi_apn'):
       INSERT subscriber_apn_ips (
         imsi,
   ...
   BEGIN
     ...
     IF found (another slot already connected first):
       ...
       INSERT subscriber_imsis (imsi, device_id, priority=imsi_slot) ON CONFLICT DO NOTHING
       IF ip_resolution IN ('imsi', 'imsi_apn'):
         INSERT subscriber_apn_ips (imsi, apn=NULL/'$apn', existing_ip) ON CONFLICT DO NOTHING
       -- iccid/iccid_apn: subscriber_iccid_ips already covers all IMSIs on this card

     ELSE (first slot to connect for this card):
       Claim one IP from ip_pool_available (SELECT FOR UPDATE SKIP LOCKED)
       → No IP: ROLLBACK, return 503

       INSERT subscriber_profiles (iccid=derived_iccid, account_name,
                                   ip_resolution=$ip_resolution)

       IF ip_resolution IN ('iccid', 'iccid_apn'):
         INSERT subscriber_iccid_ips (device_id, apn=NULL/'$apn', static_ip, pool_id)
       ...
       FOR each sibling slot in imsi_range_configs WHERE iccid_range_id = X:
         ...
         INSERT subscriber_imsis (sibling_imsi, device_id, priority=slot) ON CONFLICT DO NOTHING
         IF ip_resolution IN ('imsi', 'imsi_apn'):
           INSERT subscriber_apn_ips (sibling_imsi, apn=NULL/'$apn', static_ip)
           ON CONFLICT DO NOTHING
         -- Up to 10 slots; all share one IP
   COMMIT
...
|---|---|---|---|
| `imsi` | NULL (wildcard) | `subscriber_apn_ips` | Per IMSI |
| `imsi_apn` | APN from Access-Request | `subscriber_apn_ips` | Per IMSI |
| `iccid` | NULL (wildcard) | `subscriber_iccid_ips` | Card-level (shared by all IMSIs) |
| `iccid_apn` | APN from Access-Request | `subscriber_iccid_ips` | Card-level |
